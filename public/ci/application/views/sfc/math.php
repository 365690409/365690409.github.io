...
_this.setCurDistance(_this.initPosition);_this.addListenerAll();_this.cancelBtn.addEventListener('click',function(){_this.mobileSelect.classList.remove('mobileSelect-show');});_this.ensureBtn.addEventListener('click',function(){_this.mobileSelect.classList.remove('mobileSelect-show');var tempValue='';for(var i=0;i<_this.wheel.length;i++){i==_this.wheel.length-1?tempValue+=_this.getInnerHtml(i):tempValue+=_this.getInnerHtml(i)+_this.connector;}
_this.trigger.innerHTML=tempValue;_this.callback(_this.getIndexArr(),_this.getValue());});_this.trigger.addEventListener('click',function(){_this.mobileSelect.classList.add('mobileSelect-show');});_this.grayLayer.addEventListener('click',function(){_this.mobileSelect.classList.remove('mobileSelect-show');});_this.popUp.addEventListener('click',function(){event.stopPropagation();});_this.fixRowStyle();},setTitle:function(string){var _this=this;if(string==1){_this.titleText='<b class="mobileSelect_time"></b>请选择出发时间';}else if(string==2){_this.titleText='<b class="mobileSelect_people"></b>请选择可载人数';}else if(string==3){_this.titleText='<b class="mobileSelect_people"></b>给车主留言';}else if(string==4){_this.titleText='<b class="mobileSelect_people"></b>添加感谢费';}else{_this.titleText=string;}_this.mobileSelect.querySelector('.title').innerHTML=_this.titleText;},setStyle:function(config){var _this=this;if(config.ensureBtnColor){_this.ensureBtn.style.color=config.ensureBtnColor;}
...
'<div class="ensure pkbut pkbutsd">'+ensureText+'</div>'+
'<div class="clear20"></div>'+
'</div>'+
'</div>'+
...
_this.checkArrDeep(resultNode);_this.reRenderWheels();_this.fixRowStyle();_this.setCurDistance(_this.resetPostion(index,posIndexArr));},resetPostion:function(index,posIndexArr){var _this=this;var tempPosArr=posIndexArr;var tempCount;if(_this.slider.length>posIndexArr.length){tempCount=_this.slider.length-posIndexArr.length;for(var i=0;i<tempCount;i++){tempPosArr.push(0);}}else if(_this.slider.length<posIndexArr.length){tempCount=posIndexArr.length-_this.slider.length;for(var i=0;i<tempCount;i++){tempPosArr.pop();}}
...
_this.addListenerLi(i);})(i);}},updateWheels:function(data){var _this=this;if(_this.cascade){_this.cascadeJsonData=data;_this.displayJson=[];_this.initCascade();if(_this.initPosition.length<_this.slider.length){var diff=_this.slider.length-_this.initPosition.length;for(var i=0;i<diff;i++){_this.initPosition.push(0);}}
...
return temp;},getValue:function(){var _this=this;var temp=[];var positionArr=_this.getIndexArr();if(_this.cascade){for(var i=0;i<_this.wheel.length;i++){temp.push(_this.displayJson[i][positionArr[i]]);}}
...
_this.transitionEnd(_this.getIndexArr(),_this.getValue());if(_this.cascade){var tempPosArr=_this.getIndexArr();tempPosArr[index]=_this.getIndex(_this.curDistance[index]);_this.checkRange(index,tempPosArr);}
...
_this.clickStatus=false;_this.transitionEnd(_this.getIndexArr(),_this.getValue());if(_this.cascade){var tempPosArr=_this.getIndexArr();tempPosArr[index]=_this.getIndex(_this.curDistance[index]);_this.checkRange(index,tempPosArr);}
...
function modata(){
var myDate = new Date();
var _mh=myDate.getHours();
var _mm=myDate.getMinutes();
_mm=parseInt(_mm/5)+2;
if(_mm>=11){
_mh=_mh+1;
_mm=_mm-11;
}
...
function getdistance(d2,d3,d4,d5) {
  ...
  var arrayOfDouble1 = [];
  var arrayOfDouble2 = [];
  arrayOfDouble1.push(Math.cos(d3) * Math.cos(d2));
  arrayOfDouble1.push(Math.cos(d3) * Math.sin(d2));
  arrayOfDouble1.push(Math.sin(d3));
  arrayOfDouble2.push(Math.cos(d5) * Math.cos(d4));
  arrayOfDouble2.push(Math.cos(d5) * Math.sin(d4));
  arrayOfDouble2.push(Math.sin(d5));
  var d14 = Math.sqrt((arrayOfDouble1[0] - arrayOfDouble2[0]) * (arrayOfDouble1[0] - arrayOfDouble2[0]) +
   (arrayOfDouble1[1] - arrayOfDouble2[1]) * (arrayOfDouble1[1] - arrayOfDouble2[1]) +
   (arrayOfDouble1[2] - arrayOfDouble2[2]) * (arrayOfDouble1[2] - arrayOfDouble2[2]));
  return ((Math.asin(d14 / 2.0) * 12742001.579854401)*0.001).toFixed(2);
}
function getanalysis(d1,d2) {
	if(d1 && d2){
    d3=d1.split(","); 
    d4=d2.split(","); 
	return getdistance(d3[0],d3[1],d4[0],d4[1]);
	}
	return "";
}
