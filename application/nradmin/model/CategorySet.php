...
class CategorySet extends Controller{
     /*列表条件*/
    ...
    public function nrselect($class){
       ...
       foreach($list as $key=>$rs){
		 ...
		 $data['name_nickname']=$rs['nickname'];
		 $data['ctag_status']=$rs['status']=="normal"?'text|success|正常|||ok':'text|gray|隐藏|||remove';
		 $data['ptag_oper']='t-btn|success btn-xs||编辑|category/list#category/edit/'.$rs['id'].'|pencil|,|p-btn|danger btn-xs||确定删除此项?|category/delete/'.$rs['id'].'|trash|';
	      $list_data[]=$data;
       }
    	return [
        'thead'=>['sele_id'=>'ID','name_id'=>'ID','name_type'=>'类型','name_name'=>'名称','left_nickname'=>'昵称','left_status'=>'状态','name_oper'=>'操作'],
  		'data'=>$list_data,
  		'total'=>$list->total(),
