...
define('FONT_URL', dirname(__FILE__));
use think\Db;
use think\custom\Imgchuli;
class Img {
	...
	public function img_dz($img,$fa,$fb,$font,$black,$str,$wx,$wy=0) {
	  ...
	  foreach($sar as $k=>$v){
		if($v=="/"){
		  ...
		  $y=$y+$f_y;
          $imgchuli=new Imgchuli();
		  if(strlen($v)>6){
		  imagefttext($img,($fa/3),$fb,($x-10),$y, $black,$imgchuli->font("simsun.ttf"),$v);
		  }else{
		  imagefttext($img,$fa,$fb,$x,$y, $black,$imgchuli->font("t.ttf"),$v);
		  }
		  $i++;
		}
	  }
